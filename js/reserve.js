function reserInfo(e){
		...
		if(type == "Y"){
			$.each(data.result,function(key,val){
				...
				$("table#ROOM_CODE tbody").append(
					...
					+ "<td>" + P04_SAT_PRCE + "만원</td>"					
					+ "<td>" + P05_WEEK_PRCE + "만원</td><td>" + P05_FRD_PRCE + "만원</td>"
					+ "<td>" + P05_SAT_PRCE + "만원</td>" + 
					+ "</tr>" 
 				);
				i++;
			});
			...
			function(data){
				...
				for(var i = 0; i < arr5.length; i++){
					...
					var rsp = -(ft - lt);
                    $("tbody tr").eq(arr5[i]).prepend("<th rowspan='"+ rsp +"'>최소 위약금</th>");	
				}
			});
		}else{
			$.each(data.result,function(key,val){
	...
	function(data){
        ...
        $.each(data.result, function (key, val) {
          ...
          if (val["STD_DAY"] > 30) $("table#REFU_PRCE thead tr th:last").text("예약일 당일");
        });
        ...
        $("table#REFU_PRCE thead tr th").eq(1).remove();
        $("table#REFU_PRCE tbody tr td").eq(1).remove();
	});
}